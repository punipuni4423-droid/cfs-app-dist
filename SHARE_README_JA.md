...
Git管理版では、通常の更新は画面の App Update を使います。アプリや更新画面が起動しない場合は同じフォルダの `UPDATE_CFS_APP.cmd` を実行してください。実行中の更新があれば完了を待ち、その窓は閉じないでください。未保存の編集を保存し、他の起動窓を閉じてから実行します。詳しくは [独立Git復旧手順](GIT_RECOVERY_GUIDE_JA.md) を参照してください。
