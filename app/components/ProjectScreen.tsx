...
import {
  ...
  stripAutoGeneratedRevisionNote,
...
export default function ProjectScreen({
  ...
  // Comparison base per revision id; empty string means "the previous revision".
  const [revisionCompareBases, setRevisionCompareBases] = useState<Record<string, string>>({});
  // Snapshots are immutable once saved, so a diff can be keyed by the two
  // revision ids. Without this, editing a memo re-diffs every revision row.
  const revisionDiffCacheRef = useRef(new Map<string, RevisionChangeEntry[]>());
  // Row labels resolve against the current area and device masters, so a change
  // there invalidates every cached diff.
  useEffect(() => {
    revisionDiffCacheRef.current.clear();
  }, [project.locations, devices]);
  ...
  const revisionChangeEntries = useCallback((before: RevisionSnapshot, after: RevisionSnapshot): RevisionChangeEntry[] => {
    return buildRevisionChangeEntries(before, after, { locations: project.locations, devices });
  }, [devices, project.locations]);

  /**
   * Text shown when no cell-level change was detected. The detailed diff is
   * rendered from structured entries, so this only covers the initial revision,
   * a no-op save, and collections that only report a count.
   */
  const revisionFallbackSummary = useCallback((before: RevisionSnapshot | null, after: RevisionSnapshot): string => {
  ...
  interface RevisionDiffAtIndex {
    entries: RevisionChangeEntry[];
    summaryText: string;
    baseLabel: string;
    baseOptions: RevisionBaseOption[];
  }

  /**
   * Recomputes the diff for one revision against a chosen base. Revisions store
   * full snapshots, so any earlier revision can act as the base.
   */
  function revisionDiffAtIndex(rt: RoomType, index: number, baseRevisionId: string): RevisionDiffAtIndex {
    const revisions = rt.revisions ?? [];
    const baseOptions: RevisionBaseOption[] = [];
    for (let i = index - 1; i >= 0; i -= 1) {
      const candidate = revisions[i];
      // The immediately previous revision keeps the empty value so it stays the
      // default even after revisions are added or reordered.
      baseOptions.push(
        i === index - 1
          ? { value: "", label: `${candidate.revision || "-"} (previous)` }
          : { value: candidate.id, label: candidate.revision || "-" },
      );
    }
    if (baseOptions.length === 0) baseOptions.push({ value: "", label: "Initial" });

    const empty: RevisionDiffAtIndex = { entries: [], summaryText: "No memo.", baseLabel: "-", baseOptions };
    const revision = revisions[index];
    if (!revision) return empty;
    const after = parseRevisionSnapshot(revision.snapshot);
    if (!after) return empty;

    const explicitBaseIndex = baseRevisionId
      ? revisions.findIndex((item) => item.id === baseRevisionId)
      : -1;
    const baseIndex = explicitBaseIndex >= 0 && explicitBaseIndex < index ? explicitBaseIndex : index - 1;
    const baseRevision = baseIndex >= 0 ? revisions[baseIndex] : null;
    const before = baseRevision ? parseRevisionSnapshot(baseRevision.snapshot) : null;

    const cacheKey = `${rt.id}|${baseRevision?.id ?? "none"}|${revision.id}`;
    let entries = revisionDiffCacheRef.current.get(cacheKey);
    if (!entries) {
      entries = before ? revisionChangeEntries(before, after) : [];
      revisionDiffCacheRef.current.set(cacheKey, entries);
    }

    return {
      entries,
      summaryText: revisionFallbackSummary(before, after),
      baseLabel: baseRevision?.revision || "Initial",
      baseOptions,
    };
  }
  ...
  const setFixtures = useCallback(
    (next: FixtureMaster[]): void => {
      updateProject((p) => {
        // T-26/T-28: fixture Type transitions retroactively drive FFE on every
        // circuit (all room types) using the fixture — became FFE → check,
        // left FFE → uncheck. Matching is by fixture NAME so CSV imports
        // (which mint new ids) behave the same as in-place edits. Circuits
        // using other fixtures, including manually checked rows, stay as-is.
        const typeByName = (list: readonly FixtureMaster[]): Map<string, string> => {
          const map = new Map<string, string>();
          for (const fx of list) {
            const name = fx.fixture.trim();
            if (name !== "" && !map.has(name)) map.set(name, fx.fixtureType);
          }
          return map;
        };
        const prevTypes = typeByName(p.fixtures);
        const nextTypes = typeByName(next);
        const turnedOn: string[] = [];
        const turnedOff: string[] = [];
        for (const [name, type] of nextTypes) {
          const prev = prevTypes.get(name);
          if (type === "FFE" && prev !== "FFE") turnedOn.push(name);
          else if (type !== "FFE" && prev === "FFE") turnedOff.push(name);
        }
        let circuits = p.circuits;
        if (turnedOn.length > 0) circuits = setFfeOnCircuitsUsingFixtures(circuits, turnedOn, true);
        if (turnedOff.length > 0) circuits = setFfeOnCircuitsUsingFixtures(circuits, turnedOff, false);
        return { ...p, fixtures: next, circuits };
      });
    },
    [updateProject],
  );
  ...
  const saveRoomTypeRevision = useCallback((
    ...
    // Only the memo the user actually typed is persisted. The automatic change
    // list is recomputed from the stored snapshots on every render, so storing
    // it here would only freeze a stale copy into an editable memo field.
            ...
            <table className="mini-table revision-table">
              ...
              <tbody>
                {displayedRevisions.map(({ revision, index }) => {
                  // Revisions saved before the memo/diff split stored the generated
                  // change list in `note`; keep only what a person actually typed.
                  const storedNote = stripAutoGeneratedRevisionNote(revision.note ?? "");
                  const diff = revisionDiffAtIndex(activeRoomType, index, revisionCompareBases[revision.id] ?? "");
                  return (
                    <tr key={revision.id}>
                      <td>
                        <input
                      ...
                      <td>
                        ...
                        <div
                          ...
                          aria-label={`Detected changes for revision ${revision.revision || "saved revision"}`}
                        >
                          <RevisionDiffPanel
                            entries={diff.entries}
                            summaryText={diff.summaryText}
                            roomTypeName={activeRoomType.name}
                            targetRevisionLabel={revision.revision || "-"}
                            baseRevisionLabel={diff.baseLabel}
                            baseOptions={diff.baseOptions}
                            baseValue={revisionCompareBases[revision.id] ?? ""}
                            onBaseChange={(value) =>
