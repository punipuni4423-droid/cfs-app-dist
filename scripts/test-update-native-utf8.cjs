...
fs.writeFileSync(path.join(author, 'package.json'), '{"name":"synthetic-cfs-update-fixture","private":true}\n');
...
for (const file of ['update-cfs-app.ps1', 'cfs-local-data-preservation.ps1']) fs.copyFileSync(path.join(__dirname, file), path.join(author, 'scripts', file));
git('-C', author, 'add', '.');
...
(async () => {
  ...
  for (const test of cases) {
    ...
    assert.equal(status?.state, 'completed', `Worker did not complete: ${test.name}/${test.codePage}`);
    assert.equal(status.message, 'Docs-only update applied. No rebuild was needed.');
    ...
    assert.equal(JSON.parse(fs.readFileSync(path.join(test.folder, '.cfs-build-info.json'), 'utf8').replace(/^\uFEFF/, '')).gitSha, after);
  ...
  console.log(`PASS: old CP932 boundary rejected; ${results.length} real worker updates completed (PS5/PS7, detached, CP932/UTF8). Fixtures: ${root}`);
